...
IsleWhispers 当前工程的 `developmentRegion` 已经是 `en`，但界面、声音名称、辅助功能、错误状态和通知文案仍直接写在 Swift 中，并以简体中文为主。应用需要统一支持英文和简体中文，跟随 iOS 系统或“单独设置 App 语言”的选择；无法匹配简体中文时必须回退英文。
...
- 英文作为开发语言和默认回退语言。
- 支持 `en` 与 `zh-Hans`，完全跟随系统语言，不提供 App 内手动语言开关。
...
新增 `Localizable.xcstrings`，源语言为英文，并提供完整的 `zh-Hans` 翻译。工程保留 `developmentRegion = en`，`knownRegions` 增加 `zh-Hans`。未匹配到简体中文的系统语言使用英文资源。
...
协议正文由完整的本地化格式串生成，并通过独立的协议名称 key 定位高亮范围。英文和中文都必须包含用户协议与隐私政策高亮；点击回调及 VoiceOver 自定义操作保持一致。

如果翻译文本缺少协议名称，测试应失败，而不是静默显示不可点击的普通文本。

### 6. 辅助功能与布局

VoiceOver 的 label、hint、value 和 custom action 与可见文本使用同一语言来源。英文通常比中文更长，因此现有按钮、标题和卡片继续支持多行及 Dynamic Type；重点覆盖 320×568、390×844 和最大辅助字号。
...
- 英文与简体中文包含相同的必需 key。
- 每个 key 都有非空英文值和非空简体中文值。
- 不支持的 Locale 回退英文。
- 格式化倒计时、计数和版本信息在两种语言下符合预期。

### 模型与服务测试

- 15 个声音及 3 个分类在英文和中文下返回正确名称。
...
- 指定简体中文 Bundle 验证相同流程的关键文案。
- YYText 的两个高亮和 VoiceOver 自定义操作在英文、中文下均存在并可触发。
- 英文在小屏和最大辅助字号下不截断关键标题、按钮或倒计时内容。
